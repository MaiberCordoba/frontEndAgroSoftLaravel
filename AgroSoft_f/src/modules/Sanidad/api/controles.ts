...
export const patchControles = async (
  id: number,
  data: Partial<Controles>
): Promise<Controles> => {
...
export const getById = async (id: number): Promise<Controles> => {
  const response = await apiClient.get<Controles>(`controles/${id}/`);
  return response.data;
};
